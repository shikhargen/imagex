...
- Zoom performance is paint/compositing-sensitive with media nodes. Keep the zoom-only `viewport-zooming` class narrow: it may promote React Flow/media layers and disable decorative blur, but must not hide previews or use broad descendant wildcard rules.
