...
- Do not use paint containment on `.react-flow__node` or `.ix-node`; React Flow handles intentionally sit partly outside node borders and must remain visible. Keep paint containment scoped to internal preview/media boxes instead.
- Zoom performance is paint/compositing-sensitive with media nodes. Media previews must decode and become compositing-ready without requiring workflow switching or preview-resolution toggles. `FlowEditor` owns the post-layout media-surface refresh: after connected preview canvases and images settle, it calls `refreshPreviewSurfaces()` from `src/web/ui/flow/imaging/previewSurface.ts` so canvases redraw and bounded preview images remount against warm browser resources. Keep the zoom-only `viewport-zooming` class narrow: it may disable decorative blur, but must not hide previews, promote broad descendant trees, or use wildcard rules.
- Avoid `onlyRenderVisibleElements` for the main canvas unless profiling proves it helps; mounting/unmounting media-heavy nodes during pan/zoom can be slower than keeping stable node DOM.
...
- Edit-node previews, graph exports, and frontend downloads use the raw WebGL pipeline in `src/web/ui/flow/imaging/webglEngine.ts`, reached through `useCanvasRenderer` and the `pipeline.ts` compatibility exports. Keep this as a single shared WebGL renderer with cached source textures; do not create one WebGL context per node preview. Visible edit-node canvases should use the configured preview resolution, while full-resolution export/download paths must explicitly bypass preview caps. Output-node and image-selector previews use `PreviewImage` to cap browser preview decoding at 2048px on the long edge; keep its shared bounded-preview cache so repeated previews of the same source do not decode and downscale independently.
...
- Primitive node text fields start from built-in defaults for old workflows, but once a user renames, deletes, adds, or edits fields the node stores an authoritative managed `data.fields` list with `data.fieldsMode: 'managed'`. Keep ports, compiler field traversal, and UI field rendering aligned with `fieldDefinitionsFor()`. Prompt nodes must retain at least one text-like field; other prompt text fields can be renamed or deleted.
